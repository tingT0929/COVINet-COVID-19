# -*- coding: utf-8 -*-
"""
Created on Wed Dec 22 16:02:54 2021
...
for trainCountyIndex, testCountyIndex in countyIndex:
    ...
    save_path = path + '/result/cvCompare/onlyLSTM/cov_cv{}_factor{}'.format(cvTimes, len(stayCol))
    ...
    os.chdir(path)    
    weight_file = save_path + '/weights/lstm_county_cov_{}.h5'.format(len(stayCol))
    ...
    model.load_weights(save_path + '/weights/lstm_county_cov_{}.h5'.format(len(stayCol)))
...
for d in remain_day:
    ...
    for random_seed in range(1, cvTimes+1):
        weight_path = path + '/result/cvCompare/onlyLSTM/cov_cv{}_factor{}'.format(random_seed, len(stayCol))
        model = model_from_json(open(weight_path + '/weights/lstm_county_cov_{}.json'.format(len(stayCol))).read())
        model.load_weights(weight_path + '/weights/lstm_county_cov_{}.h5'.format(len(stayCol)))
...
cvResult.to_csv(path + '/result/cvCompare/onlyLSTM/cvMetric.csv', index = False)
