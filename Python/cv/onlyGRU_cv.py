# -*- coding: utf-8 -*-
"""
Created on Wed Dec 22 16:08:41 2021
...
for trainCountyIndex, testCountyIndex in countyIndex:
    ...
    save_path = path + '/result/cvCompare/onlyGRU/cov_cv{}_factor{}'.format(cvTimes, len(stayCol))
    ...
    os.chdir(path)    
    weight_file = save_path + '/weights/gru_county_cov_{}.h5'.format(len(stayCol))
    
    model, train_main_x, train_aux_x, train_y, \
        val_main_x, val_aux_x, val_y = training_GRU_model(trainMainDF, trainAuxDF, trainYDF,
                                                           valMainDF, valAuxDF, valYDF,
                                                           factorColname, save_path, weight_file,
                                                           isTrain = isTrain)

    model.load_weights(save_path + '/weights/gru_county_cov_{}.h5'.format(len(stayCol)))
    json_string = model.to_json()
    open(save_path + '/weights/gru_county_cov_{}.json'.format(len(stayCol)), 'w').write(json_string)
...
for d in remain_day:
    ...
    for random_seed in range(1, cvTimes+1):
        weight_path = path + '/result/cvCompare/onlyGRU/cov_cv{}_factor{}'.format(random_seed, len(stayCol))
        model = model_from_json(open(weight_path + '/weights/gru_county_cov_{}.json'.format(len(stayCol))).read())
        model.load_weights(weight_path + '/weights/gru_county_cov_{}.h5'.format(len(stayCol)))
...
cvResult.to_csv(path + '/result/cvCompare/onlyGRU/cvMetric.csv', index = False)
