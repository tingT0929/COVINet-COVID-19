...
for trainCountyIndex, testCountyIndex in countyIndex:
    ...
    ############ predict train
    #########################
    trainPredict = model.predict(train_main_x)#预测训练集
    ...
    ### val
    valPredict = model.predict(val_main_x)#预测val集
    ...
    for d in remain_day:
        ...
        testPredict = model.predict(test_main_x)#预测训练集
        # inverse scaler
        ...
        ### val
        test_valPredict = model.predict(test_val_main_x)#预测训练集
        # inverse scaler
...
for d in remain_day:
    ...
    for random_seed in range(1, cvTimes+1):
        ...
        trainPredict = model.predict(train_main_x)#预测训练集
        valPredict = model.predict(val_main_x)#预测验证集
