...
def get_cv_train_data_v2(dataset, latLongDF, scalerCOVID, scalerFactor, scalerLatLong, look_back, stateCounty, remain_day = None):
    dataset = dataset.sort_values(by = ['date'], axis = 0)
    dataset.reset_index(inplace = True, drop = True)
    ...
    trainDF = dataset[dataset.StateCounty.isin(stateCounty)]
    if remain_day:
        valDF = trainDF[trainDF['date'].isin(tempTime[-remain_day:])]
    else:
        valDF = trainDF[trainDF['date'].isin(tempTime[-look_back:])]
        
    trainDF.drop(['StateCounty', 'date', 'county', 'state'], axis = 1, inplace = True)
    valDF.drop(['StateCounty', 'date', 'county', 'state'], axis = 1, inplace = True)
    trainDF.fillna(trainDF.mean(), inplace = True)
    valDF.fillna(valDF.mean(), inplace = True)
    
    colname = list(trainDF.columns.values)
    colname.remove('LON')
    colname.remove('LAT')
    trainMainDF = trainDF[colname[:2*look_back]]
    trainAuxDF = trainDF[colname[2*look_back:-2] + ['LON', 'LAT']]
    trainYDF = trainDF[colname[-2:]]
    valDF.reset_index(drop = True, inplace = True)
    valMainDF = valDF[colname[:2*look_back]]
    valAuxDF = valDF[colname[2*look_back:-2] + ['LON', 'LAT']]
    valYDF = valDF[colname[-2:]]
    ...
    valAuxDF.iloc[:, -2:] = scalerLatLong.transform(valAuxDF.iloc[:, -2:])
    valYDF.iloc[:, :] = scalerCOVID.transform(valYDF.iloc[:, :])
    return trainMainDF, trainAuxDF, trainYDF, valMainDF, valAuxDF, valYDF

def get_cv_test_data(testMainDF, testAuxDF, testYDF, testValMainDF, testValAuxDF, testValYDF, factorColname):
    testAuxDF = testAuxDF.drop(factorColname, axis = 1) 
    testValAuxDF = testValAuxDF.drop(factorColname, axis = 1) 
    
    test_main_x = testMainDF.values
    test_aux_x = testAuxDF.values
    test_y = testYDF.values
    
    test_val_main_x = testValMainDF.values
    test_val_aux_x = testValAuxDF.values
    test_val_y = testValYDF.values
    
    test_main_x = test_main_x.reshape(test_main_x.shape[0], 1, test_main_x.shape[1])
    test_val_main_x = test_val_main_x.reshape(test_val_main_x.shape[0], 1, test_val_main_x.shape[1])
    return test_main_x, test_aux_x, test_y, test_val_main_x, test_val_aux_x, test_val_y
