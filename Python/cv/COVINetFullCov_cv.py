...
##################################
## CV 10 fold
stayCol = [22, 23, 25, 27, 30, 32, 33, 34, 42, 43]
stayCol.sort()
...
for trainCountyIndex, testCountyIndex in countyIndex:
    ...
    if noNewYork == 1:
        save_path = path + '/result/cvCompare/lstmGRU10Var/noNY/cov_cv{}_factor{}'.format(cvTimes, len(stayCol))
    else:
        save_path = path + '/result/cvCompare/lstmGRU10Var/withNY/cov_cv{}_factor{}'.format(cvTimes, len(stayCol))
    ny_path = path + '/result/cvCompare/lstmGRU10Var/withNY'
    ...
    for d in remain_day:
        ...
        testPredict = model.predict([test_main_x, test_aux_x,])
        # inverse scaler
...
for d in remain_day:
    ...
    for random_seed in range(1, cvTimes+1):
        if noNewYork == 1:
            weight_path = path + '/result/cvCompare/lstmGRU10Var/noNY/cov_cv{}_factor{}'.format(random_seed, len(stayCol))
        else:
            weight_path = path + '/result/cvCompare/lstmGRU10Var/withNY/cov_cv{}_factor{}'.format(random_seed, len(stayCol))
...
cvResult.to_csv(path + '/result/cvCompare/lstmGRU10Var/cvMetric.csv', index = False)